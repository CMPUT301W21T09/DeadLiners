...
public class SubscribeExperimentListActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_subscribe_experiment);
        ...
        userCollectionReference.document(currentUid).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    if (document.exists()) {
                        ...
                        if (subscribe.getSubscribe()!=null&& subscribe.getSubscribe().size()>0){
                            collectionReference.addSnapshotListener(new EventListener<QuerySnapshot>() {
                                ...
                                public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                                    ...
                                    for(QueryDocumentSnapshot doc: value) {
                                        ...
                                        for (int i = 0; i < subscribe.getSubscribe().size(); i++) {
                                            if(expName.toUpperCase().contains(subscribe.getSubscribe().get(i).toUpperCase())) {
        ...
        expSearchList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (owner.equals(currentUid)) {
                    Intent intent = new Intent(SubscribeExperimentListActivity.this, experimentInfo_owner.class);
                    intent.putExtra("experiment",experiment);
                    intent.putExtra("uid",currentUid);
                    startActivity(intent);
                } else {
                    Intent intent = new Intent(SubscribeExperimentListActivity.this, experimentInfo_user.class);
                    intent.putExtra("experiment",experiment);
                    intent.putExtra("uid",currentUid);
                    startActivity(intent);
                }
